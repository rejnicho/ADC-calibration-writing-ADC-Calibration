...
This method was developed for use in Rubin Observatory's LSST Camera but could be applied to other ADCs. 
...
The calibration recovers important quantities of the ADCs, including differential and integral nonlinearity which fall within specifications from the manufacturer, Analog Devices (\cite{AnalogDevices}). 
...
The camera features over 3.2 gigapixels arranged over 189 science grade charge coupled devices (CCDs) manufactured by two vendors Teledyne (e2V) and Imaging Technology Laboratory (ITL). 
...
The specific CCDs used for the LSST Camera are 100\textmu m thick, a thickness selected to maximize infrared response and minimize diffusion(\cite{OConnor2006}).  
...
The camera was later assembled at SLAC, and the full focal plane was studied in electro optical (EO) testing at SLAC to measure critical characteristics such as the bias, saturation, and linearity, (\cite{Roodman2024}) for a CCD by CCD basis.
...
In this work we will motivate and produce the missing DNL ADC calibration for the LSST Camera.
We examine each of the over 3000 Analog Devices 18-bit, 1 MSPS PulSAR ADCs (\cite{AnalogDevices})in the LSST Camera, and demonstrate each ADC falls within the specifications from the manufacturer. 

The cumulative binning method  is build and examined in the following sections. 
...
THe results of the method, including highlights of differences due to calirbating run type is detailed in Section \ref{sec:results}.
Section \ref{sec:Future} details how to apply this calibration, and includes a sample look-up table (LUT) for one ADC. 
Finally, the appendix notes additional information learned from this calibration to guide the reader in conducting a calibration themselves. 

\subsection{Definition of Differential and Integral Nonlinearity}
\label{sec: DNL_INL_Def}

An analog to digital converter (ADC) is an electronic component which converts a continuous signal, like an electric potential, to an integer value corresponding to that amount. 
The digital signal is measured in integer analog to digital units (ADU). 
Each ADC has an unique structure which maps specific potentials to ADU, meaning specific ranges of potential yield the same ADU. 
The limits for the potential mapped to each ADU are called the edges, with the left edge specifying the lower bound and the right edge the upper bound. 
...
The difference between the ideal and mock ADC can be seen on the x axis, where the input or voltage the ADC is digitizing. 
In the ideal ADC the edges of the input are integer values, while in the mock ADC these take real value edges. 
The mock ADC shows the presence of non uniform bin sizes which are not present in the ideal ADC, which more accurately depicts a true ADC. 
...
\label{eq: DNL}
\end{equation}
where $w_{ideal}$ and $w_{ADC}$ are the binwidths, the difference of right an left edge from Figure \ref{fig:idealADC}, such $w_{ideal}$ = 1.  
...
For an ADC calibration, we must use true data digitized by the ADCs. 
This data comes from two separate EO Testing runs completed at SLAC, the earlier 13144 a flat pairs run, and a later specality ramp run 13549. 
Both these runs will examine all ADU used in LSST camera, and will differ ADC to ADC. 
In order to conduct our calibration, we understand a few requirements exist. 
In this section we will examine them as follows: having the entire dynamic range of the ADC, an even representation of each region of the ADC, image trimming, and generating a model ADC using a Savisky-Golay filter. 

\subsubsection{Full Dynamic CCD range}
\label{sec:RangeADC}

The LSST Camera uses only a subsection of the full dynamic range of the its ADCs.
The bounds of the signal into the ADCs are directly related to the bias and saturation of the CCDs themselves. 
Due to the photoelectric conversion of light on the CCDs, a range of incident flux to the CCDs is necessary to look at the use range of the ADCs.
For the LSST camera different collections of exposures, called runs, probe the behavior of the sensors at different illumination level (as set through exposure time). 
These runs, completed at SLAC during EO testing These runs, completed at SLAC as EO Testing, are designed with specific calibrations and characterizations as their focus, which help inform ISR corrections(\cite{PlazasMalagon2025}). 

The first dataset covering the range of ADU codes we wish to characterize is 13144. 
When all exposures are compiled into a single histogram in counts per ADU, we examine the distribution of codes represented by the ADC.  
...
The most present is the right skew of the signal, which corresponds to a high density of exposures with low exposure time, and therefore low ADU.
...
The problem is amplified by Poisson spreading of the signal about its mean which increases as a function of flux. 
...
We also developed a specific run for this calibration, a ramp run 13549. 
The ramp run was designed to look at the ADC signal without superimposed run structure. 
This was accomplished by allowing the sensors to continue to integrate charge while the focal plane was read out, resulting in exposures with a base illumination plus a ramp upwards. 
...
After the run is selected, we must trim all the exposures in the run to remove codes from pixels on the outside edges of the CCDs.
These pixels correspond to the regions near the gaps between adjacent CCDs,
On these edge locations, the pixels have diverging electric fields.
This manifests as a small peak near the bias level in the combined histogram of all exposures. 
Since the voltage is proportional to the electric field, the signal in these regions will be reported as less than its true value. 
Iterative testing of 0-12 pixel removal demonstrated 2 pixels was sufficient in removing this defect.
For consistency we trim all exposures in both the ramp and flat pairs runs. 
The result of each trimmed exposure is then complied for the ADC calibration, perserving as many ADU signals as possible.
In additional, we note picture frame removal has also been studied as part of the ISR for LSST science image preparation.
The values used in ISR and this calibration are different.  


\subsubsection{Savisky-Golay Filter}
\label{sec:Filter}
...
The authors would like to thank Yousuke Utsumi and the LSST Camera team for helpful comments and guidance on early drafts of this work. This work is ... 
\end{acknowledgments}
